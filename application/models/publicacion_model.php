<?php
	class Publicacion_model extends CI_Model{
		...
		function listar($where = "",$fields = "*",$vista = ""){
			if($where != ""){
				$this->db->where($where);
			}
			return $this->db->get($vista);
		}
		...
		function actualizarPublicacionCarrera($data_beca,$data_carrera){
			for ($i=0; $i < count($data_carrera); $i++) {
				$this->db->where("carrera_id",$data_carrera[$i]);
				$this->db->where("publicacion_id",$data_beca["publicacion_id"]);
